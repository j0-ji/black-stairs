...
func _update_upgrade_multiplier(upgrade_name : String) -> void:
	if upgrade_name == "speed" and UpgradeManager.has_upgrade("speed"):
		...
		max_stamina = base_stamina + UpgradeManager.get_upgrade_stat_adapter("stamina") as int
...
func _on_died():
	EventBus.play_transition.emit("", "Defeated", "As he roamed the dungeon the trader found you critically injured and brought you back to the village. What a happy coincidence.")
