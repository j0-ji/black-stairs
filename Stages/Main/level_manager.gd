...
var with_initial_location : bool = true

func _ready() -> void:
	SceneManager.update_player_position.connect(	_set_player_on_bed_spawn_point)
	if with_initial_location:
		if SaveGameManager.global_data.current_location == null or SaveGameManager.global_data.current_location.is_empty():
...
func _go_to_next_dungeon_level() -> void:
	...
	SceneManager.load_location(SaveGameManager.global_data.current_location)
	await get_tree().process_frame
	SaveGameManager.global_data.current_level += 1
	_set_player_on_entrance_spawn_point()
	_set_listener_for_exit()
...
func _set_player_on_entrance_spawn_point() -> void:
	...
	GameManager.player.position = location.entrance.spawn_point
...
func _set_player_on_bed_spawn_point() -> void:
	...
	GameManager.player.position = location.bed.spawn_point
